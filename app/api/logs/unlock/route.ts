/**
 * Opens the trace page for one browser.
 *
...
async function handlePost(req: Request) {
    ...
    if (!logsPasswordConfigured()) {
        return NextResponse.json(
            { error: "LOGS_PASSWORD n'est pas défini dans .env : la trace est fermée." },
            { status: 503 }
        );
    }
    ...
    if (!checkLogsPassword(password)) {
        const count = (record?.count ?? 0) + 1;
        attempts.set(auth.id, {
            count,
            until: count >= MAX_ATTEMPTS ? Date.now() + LOCKOUT_MS : 0,
        });
        logger.warn("Rejected trace unlock", { action: "logs-unlock-failed", userId: auth.id, count });
        return NextResponse.json({ error: "Mot de passe incorrect." }, { status: 401 });
    }

    attempts.delete(auth.id);
    logger.info("Trace unlocked", { action: "logs-unlock-success", userId: auth.id });

    const { value, expiresAt } = await issueLogsCookie();
    const res = NextResponse.json({ ok: true });
    res.cookies.set({ ...logsCookieOptions(expiresAt), value });
    return res;
}

export async function POST(req: Request) {
    return withRequestContext("logs/unlock", () => handlePost(req));
}
